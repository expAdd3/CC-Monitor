use monitor_domain::{
    reduce, reduce_from_checkpoint, reduce_from_checkpoint_at, AgentEvent, AgentKind, Confidence,
    EventId, EventSource, NotificationKind, ReducerCheckpoint, SessionId, SessionLifecycle,
...
#[test]
fn checkpoint_resume_is_observationally_equal_to_one_full_reducer_replay() {
    let events = [
        event_at(
            "checkpoint-prompt-1",
            EventSource::Hook,
    ...
    let expected = reduce(events.clone());
    for split in 0..=events.len() {
        let first = reduce_from_checkpoint(
            ReducerCheckpoint::default(),
            events[..split].iter().cloned(),
        );
        let resumed = reduce_from_checkpoint(first.checkpoint, events[split..].iter().cloned());

        assert_eq!(
            resumed.reduction.projection, expected.projection,
            "projection differs at split {split}"
        );
        assert_eq!(
            first
                .reduction
                .notifications
                .into_iter()
                .chain(resumed.reduction.notifications)
                .collect::<Vec<_>>(),
            expected.notifications,
            "notifications differ at split {split}"
        );
    }
}

#[test]
fn checkpoint_resume_after_time_derived_transcript_idle_matches_full_replay() {
    let events = [
        event_at(
            "checkpoint-transcript-thinking",
            EventSource::Transcript,
    ...
    let expected = reduce_from_checkpoint_at(
        ReducerCheckpoint::default(),
        events.iter().cloned(),
        240_000,
    );
    for split in 0..=events.len() {
        let prefix_now = if split == 1 { 40_000 } else { 240_000 };
        let prefix = reduce_from_checkpoint_at(
            ReducerCheckpoint::default(),
            events[..split].iter().cloned(),
            prefix_now,
        );
        if split == 1 {
            assert_eq!(
                prefix.reduction.projection.as_ref().unwrap().turn_state,
                TurnState::Waiting
            );
        }
        let resumed =
            reduce_from_checkpoint_at(prefix.checkpoint, events[split..].iter().cloned(), 240_000);

        assert_eq!(
            resumed.reduction.projection, expected.reduction.projection,
            "time-derived projection differs at split {split}"
        );
        assert_eq!(
            prefix
                .reduction
                ...
                .chain(resumed.reduction.notifications)
                .collect::<Vec<_>>(),
            expected.reduction.notifications,
            "time-derived notifications differ at split {split}"
        );
    }
}
